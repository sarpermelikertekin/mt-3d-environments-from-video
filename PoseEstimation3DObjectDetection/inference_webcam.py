...
def main():
    ...
    class_to_name2 = {0: "phone"} #Linemod use a single class with a name of the Linemod objects
    ...
    class_to_3d_bboxes = {class_idx: name_to_3d_bboxes[name] for class_idx, name in class_to_name2.items()} 
    ...
    while True:
        ...
        for label in labels:
            if label == 46: # Cup
                ...
                index = np.where(labels == 46)

                cv2.putText(original_image, str(np.round(rotations[index][0][0],2)), (350,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv2.putText(original_image, str(np.round(rotations[index][0][1],2)), (350,90), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv2.putText(original_image, str(np.round(rotations[index][0][2],2)), (350,130), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                
                cv2.putText(original_image, str(np.round(translations[index][0][0],2)), (500,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                cv2.putText(original_image, str(np.round(translations[index][0][1],2)), (500,90), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                cv2.putText(original_image, str(np.round(translations[index][0][2],2)), (500,130), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
